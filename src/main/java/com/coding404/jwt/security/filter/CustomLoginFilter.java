...
public class CustomLoginFilter extends UsernamePasswordAuthenticationFilter {
	...
	//클라이언트에서 포스트 형태로 /login로 들어오면 실행됩니다
	
	private AuthenticationManager authenticationManager;
	
	
	//생성될때 AuthenticationManager를 생성자 매개변수로 받습니다
	...
	public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response)
		...
		//로그인처리 - 로그인시도하는 사람은 반드시 form 타입으로 전송 (JSON형식도 받을 수 있다 + 제이슨매핑처리)
		...
		System.out.println("내가 실행되었으면, 로그인 성공: " + authentication);
	...
	@Override
	   protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response, FilterChain chain,
		...
		System.out.println("로그인 이후 인증 객체: "+authResult);
		
		MyUserDetails principal = (MyUserDetails)authResult.getPrincipal();
		
		String token = JWTService.createToken(principal.getUsername()); //회원아이디
		
		response.setContentType("text/html; charset=UTF-8;");
		response.setHeader("Authorization", "Bearer "+token); //Bearer + token => 가장 기본이 되는 토큰
